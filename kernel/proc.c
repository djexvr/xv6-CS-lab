...
// Returns a runnable proc of the highest priority
// Return 0 if no runnable process are available
// otherwise returns a process of the highest priority
// while holding the proc lock and the prio_list lock
struct proc*
pick_highest_priority_runnable_proc(){
  for (int i = 0; i < 10 ; i++) {
    struct list_proc* list_p;
    acquire(&prio_lock);
    list_p = prio[i];
    while(list_p != 0){
      acquire(&list_p->p->lock);
      if (list_p->p->state == RUNNABLE) {
       return list_p->p;
      }
      release(&list_p->p->lock);
      list_p = list_p->next;
    }
    release(&prio_lock);
  };
...
scheduler(void)
{
  ...
  for(;;){
    ...
    if (p!=0) {
        ...
        // Move p to the end of the prio list
        struct list_proc* tail;
        struct list_proc* new_tail = bd_malloc(sizeof(struct list_proc));

        remove_from_prio_queue(p);

        tail = prio[p->priority];
        new_tail->p = p;
        new_tail->next = 0;

        if (tail == 0){
          prio[p->priority] = new_tail;
        } else {
          while(tail->next != 0) {
           tail = tail->next;
          };
